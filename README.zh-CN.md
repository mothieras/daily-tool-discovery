...
- **信任先于相关。** 每个候选都会被打成一档 —— `trusted` / `review` / `reject` —— 依据是
  ...
  会被隔离。
- **口味可配置。** "什么算相关"写在 `config/profile.toml` 里,不在代码里。改类目、信号标签、
  权重和源,就能**把这个工具重定向到任意领域** —— 见 `config/profiles/web-frontend.example.toml`。
...
本项目仅依赖标准库、自包含,所以可以零安装直接跑。把这个文件夹放/克隆到任意位置
(或放进 agent 的 skill 目录),确保 `python3` 是 3.11+ 且已 export `GITHUB_TOKEN`,
然后运行自带入口:
...
`run.py` 既能在包位于 `src/` 下时工作(本仓库),也能在包与 `run.py` 平铺时工作
(skill bundle)。状态存在 `~/.daily-tool-discovery`(可用 `DAILY_TOOL_DISCOVERY_HOME`
覆盖);首次运行会把示例 profile 和种子拷到那里,简报写到
`~/.daily-tool-discovery/briefings/<今天>.md`。`dry-run`、`save`、`deny`、`feedback`
也以同样方式运行(`python3 run.py dry-run`)。
...
默认读取 `config/profile.toml`(若存在),否则读 `config/profile.example.toml`。
发现过程综合三条输入:

- `seeds/manual.jsonl`:朋友推荐和你已知的好口味种子。服务器安装脚本首次安装时会把
  `seeds/manual.example.jsonl` 拷到这里。`discover` 把它们当作口味参考,并把种子 URL 从当日
  候选列表中过滤掉。
...
口味是数据,不是代码。`config/profile.toml`(回退到 `config/profile.example.toml`)定义了一切:
